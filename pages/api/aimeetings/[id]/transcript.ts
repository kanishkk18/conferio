...
//   }
// } was working perfectly before adding realtime transcription

// pages/api/aimeetings/[id]/transcript.ts  (MODIFIED)
import type { NextApiRequest, NextApiResponse } from 'next';
import { getServerSession } from 'next-auth/next';
import { authOptions } from '@/lib/auth';
import { prisma } from '@/lib/prisma';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { id } = req.query;
  const { lang } = req.query; // ?lang=hi for Hindi version
  ...
  try {
    ...
    // If language requested and not 'en' or 'original', try to get translated version
    if (lang && typeof lang === 'string' && lang !== 'en' && lang !== 'original') {
      const translated = await prisma.meetingTranscript.findFirst({
        where: {
          meetingId: id,
      ...
      if (translated) {
        return res.status(200).json({
          transcript: JSON.parse(translated.content),
      ...
      return res.status(202).json({
        message: `Translation for ${lang} is not ready yet. Processing...`,
        status: 'translating',
      });
    }

    // ... rest of your existing transcript logic (original language) ...
    let formattedTranscript = null;
    let transcriptStatus = 'unavailable';
