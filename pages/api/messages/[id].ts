...
export default async function handler(
  ...
  if (req.method === "DELETE") {
    try {
      ...
      const isOwner = message.sender.userId === session.user.id;
      const canModerate = CAN_MODERATE.includes(requestingMember.role);
  ...
  if (req.method === "PATCH") {
    try {
      ...
      if (message.sender.userId !== session.user.id) {
        return res.status(403).json({ error: "Not authorized to edit this message" });
      }
