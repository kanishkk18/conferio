...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === "POST") {
      if (existing) {
        result = note !== undefined
          ? await prisma.messageBookmark.update({ where: { id: existing.id }, data: { note } })
          : existing;
        isBookmarked = true;
      } else {
        result = await prisma.messageBookmark.create({
          data: { messageId, userId: session.user.id, note: note || null },
        });
        isBookmarked = true;
      }
    } else {
    ...
    return res.status(200).json({ bookmarked: isBookmarked, bookmark: result });
  } catch (error) {
    console.error("[BOOKMARK]", error);
    return res.status(500).json({ error: "Internal Error" });
  }
