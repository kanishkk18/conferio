...
const ErrorMsg: FC<PropsWithChildren> = ({ children }) => {
    return (
        <span className="absolute right-3 -top-2.5 whitespace-nowrap px-1 text-sm text-danger  sm:top-2.5 sm:right-4" aria-label="Button">
            {children}
        </span>
    );
};
        ...
        <fieldset className={`flex flex-col text-mid-grey dark:text-white ${className ?? ''}`}>
            ...
            <div className="relative" aria-label="Button">
                ...
                <PopoverEl anchorWidth={true}>
                    <div className="absolute top-12 w-full rounded-md p-4 bg-white dark:bg-neutral-800" aria-label="Button">
                        <ul ref={ulRef} className="gap-y-2">
                            {props.options.map((option) => (
                                <li
                                ...
                                >
                                    <button  type="button" onClick={handleOptionSelect}
                                    onKeyDown={handleOptionKeyDown} tabIndex={0} aria-label="Button">
                                        {option}
                                    </button>
                                </li>
                            ))}
...
const MultiValueInput: FC<MultiValueInputProps> = (props) => {
    ...
    return (
        <div className={props.className ?? ''} aria-label="Button">
            <DndContext
