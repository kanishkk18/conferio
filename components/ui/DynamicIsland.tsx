...
import { PhoneCall, PhoneCallIcon } from "../animate-ui/icons/phone-call"
...
import { TimeEntry } from '@/components/time-tracking/TimerWidget';
...
function DynamicIslandInner({
  ...
  return (
    <>
      <div ref={buttonRef} className=" z-10 flex justify-center gap-0.5 px-0.5 text-[#B4B4B4] !w-auto">
        {dockItems.map(({ key, icon }) => (
          <AnimateIcon animateOnHover key={key} className="!w-auto">
            <button
              type="button"
              className=" flex relative size-6 px-1 cursor-pointer hover:dark:bg-[#181818] hover:bg-[#fff] items-center justify-center rounded-md w-auto "
              onClick={() => {
                if (view !== key) {
...
function Calls() {
  ...
  return (
    <FloatingPanelContent className=" flex !-ml-52 w-72 items-center bg-white dark:bg-[#000] overflow-y-auto">

        {/* Show incoming call modal */} 
        {incomingCall && !isInCall && <IncomingCallModal />}

        <Tabs defaultValue="members" className="px-0 py-2.5 w-full">

          <div className=" flex justify-between items-start px-3">

            <TabsList className="bg-white dark:bg-[#111] border dark:border-[#333] h-fit !px-0 rounded-md">
              <TabsTrigger value="members" className="data-[state=active]:dark:bg-[#222] h-fit w-fit !px-1.5 rounded-md rounded-r-none">
                <Users className="mr-1 h-3 w-3" />
               <p className="text-xs">Members</p>
              </TabsTrigger>
              <TabsTrigger value="history" className="data-[state=active]:dark:bg-[#222] h-fit w-fit !px-1.5 rounded-md rounded-l-none">
          ...
          <TabsContent value="members" className="space-y-4 !px-0">
            <TeamMemberList />
          </TabsContent>

          <TabsContent value="history" className="space-y-4 !px-0">
            <CallHistory />
          </TabsContent>
        </Tabs>
  ...
  return (
    <div className="grid">
      {members.map((member) => (
        <div
          key={member.id}
          className="group relative overflow-hidden py-1 transition-all hover:shadow-xl dark:hover:bg-[#111]"
        >
          {/* Avatar & Info */}
          <div className=" flex items-center gap-4 px-2">
            {member.image ? (
              <img
                src={member.image}
                alt={member.name}
                className="h-6 w-6 rounded-full object-cover ring-2 ring-gray-700"
              />
            ) : (
              <div className="flex h-6 w-6 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-purple-600 text-md font-semibold text-white">
