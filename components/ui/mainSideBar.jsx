...
//   <DropdownMenu>
//         <DropdownMenuTrigger >
//            <button type="button" className="data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground border dark:border-[#292929] dark:bg-black rounded-sm px-1 py-1 flex justify-center items-center gap-1.5"
//             >
...
//         <DropdownMenuTrigger>
//            <button type="button"
//               className="data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground border-none px-0 flex justify-center items-center gap-1 ml-1"
//             >
//               <Avatar className="h-5 w-5 rounded-sm">
//                 <AvatarImage src={data?.user?.image} alt={data?.user?.name} />
//                 <AvatarFallback className="rounded-md">CN</AvatarFallback>
...
import Image from 'next/image';
import {
  LayoutDashboard, MessageCircleMore, UsersRound, FileText,
  SquareKanban, HardDrive, CalendarCheck2, Video, Bot,
  ListTodo, Music, Disc, Settings, Calendar,
} from 'lucide-react';
...
import { ChartSpline } from "@/components/animate-ui/icons/chart-spline";
import {
  FolderIcon, FolderPlusIcon, RectangleStackIcon, UserCircleIcon,
} from '@heroicons/react/24/outline';
import { useSession } from 'next-auth/react';
import { useTranslation } from 'next-i18next';
import { useRouter } from 'next/router';
...
export default function Mainsidebar() {
  ...
  const links = [
    {
      label: 'Dashboard',
      href: '/dashboard',
      icon: <LayoutDashboard className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Meetings',
      href: '/meetings/page',
      icon: <Video className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Chat',
      href: '/chat',
      icon: <MessageCircleMore className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Members',
      href: teamSlug ? `/members/${teamSlug}` : '/members',
      icon: <UsersRound className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Scrumboard',
      href: '/scrumboard',
      icon: <SquareKanban className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Docs',
      href: '/docs',
    ...
    {
      label: 'Workload',
      href: teamSlug ? `/teams/${teamSlug}/workload` : '/workload',
      roles: ["ADMIN", "OWNER", "MANAGER"],
      icon: <Users className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Timesheet',
      href: '/time-tracking',
      icon: <AlarmClock className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Drive',
      href: '/drive/page',
      icon: <HardDrive className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Events',
      href: '/events/page',
      icon: <CalendarCheck2 className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'AI',
      href: 'https://ai.collabium.in',
      icon: <Bot className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Calendar',
      href: '/calendar/page',
      icon: <Calendar className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Clips',
      href: '/clips',
      icon: <Disc className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: 'Whiteboard',
      href: teamSlug ? `/team/${teamSlug}/whiteboard` : '/',
      icon: <ChartSpline className="text-neutral-700 rotate-90 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
    {
      label: "Scheduled",
      href: "/scheduled/page",
    ...
    },
    {
      label: 'Settings',
      href: '/settings/page',
      icon: <Settings className="text-neutral-700 dark:text-white/70 size-[1.12rem] flex-shrink-0" />,
    },
  ];
  ...
  return (
    <div className={cn('rounded-none flex flex-col md:flex-row bg-gray-100 dark:bg-[#121212] !overflow-hidden', 'h-screen w-fit')}>
      <Sidebar open={open} setOpen={setOpen}>
        <SidebarBody className="justify-between gap-y-4">
          <div className="flex flex-col flex-1 overflow-y-auto overflow-x-hidden thin-scrollbar">
            {open ? <Logo /> : <LogoIcon />}
...
export const Logo = () => (
  <Link href="/" className="flex items-center gap-x-2">
    <Image
      ...
      width={1000}
    />
    <span className="dark:text-white text-black font-bold text-md">Conferio</span>
  </Link>
);

export const LogoIcon = () => (
  <Link href="/" className="font-normal flex gap-x-2 items-center text-sm text-black relative z-20">
    <img
      src="/logo-transparent.png"
      alt="logo"
      className="text-black p-1 bg-black rounded-md h-[28px] w-auto"
...
const TeamDropdown = () => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className='max-w-[80%] min-w-0'>
        
        <button type="button" className=" data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground border dark:border-[#292929] dark:bg-black rounded-sm px-1 py-1 flex justify-center items-center gap-2.5">
          <Avatar className="h-5 w-5 rounded-sm">
            <AvatarImage src={data?.user?.image} alt={data?.user?.name} />
            <AvatarFallback className="rounded-md">CN</AvatarFallback>
          </Avatar>
          <div className="grid flex-1 text-left text-xs leading-3 capitalize">
