...
//   const { startCapture, stopCapture } = useAudioCapture(localStream, socket, meetingId);
...
//   return (
//     <div
//       className="fixed inset-0 flex flex-col bg-[#191919] select-none overflow-hidden"
...
//       {/* ── Top bar ── */}
//       <div className="h-14 flex items-center justify-between px-4 bg-transparent backdrop-blur-md border-b border-neutral-800/50 flex-shrink-0 z-20 relative">
...
//                 transition={{ type: 'spring', damping: 25 }}
//                 className="flex flex-col border mr-4 mt-6 bg-[#111] border-[#333333] rounded-xl overflow-hidden flex-shrink-0"
//               >
//                 {showParticipants && (
//                   <ParticipantsPanel
...
//           <div className="flex justify-center items-center bg-black rounded-full !divide-y-0 !divide-x-2 !divide-[#222] overflow-hidden h-fit border border-[#333]">
//           <CtrlBtn onClick={toggleMute} active={!isMuted} danger={isMuted}
//             icon={isMuted ? MicOff : Mic} label={isMuted ? 'Unmute' : ''} />

//           <CtrlBtn onClick={toggleVideo} active={isVideoOn} danger={!isVideoOn}
//             icon={isVideoOn ? Video : VideoOff} label={isVideoOn ? '' : 'start video'} />
...
//            <button type="button"
//           onClick={() => { setShowChat((v) => !v); setShowParticipants(false); setUnreadChats(0); }}
//           className={`relative flex items-center px-2 py-1.5 text-sm transition-colors ${showChat ? ' text-white' : 'text-neutral-300'}`}
//         >
//           <MessageSquare className="size-4" />
//             {unreadChats > 0 && (
//               <span className="absolute -top-1 -right-1 size-4 text-[9px] text-white flex items-center justify-center font-bold">
//           {unreadChats > 9 ? '9+' : unreadChats}
//         </span>
//             )}
//       </button>
...
//           <button type="button"
//             onClick={raiseHand}
//             className={`flex bg-transparent items-center gap-1 px-1.5 py-2 text-[10px] font-medium transition-all ${isHandRaised
//                 ? ' text-yellow-400 border border-yellow-500/30'
//                 : ' text-neutral-400 hover:bg-neutral-700 hover:text-white border border-transparent'
//               }`}
//           >
//             <motion.span
//               animate={isHandRaised ? { rotate: [0, 15, -5, 15, 0] } : {}}
//               transition={{ repeat: isHandRaised ? Infinity : 0, duration: 1.2 }}
//               className="size-4 leading-none bg-transparent"
//             >
...
//                   className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 border border-neutral-800 rounded-2xl p-3 shadow-2xl z-50"
//                 >
...
//             </AnimatePresence>
//           </div>
// <button onClick={startCapture}>🌐 Enable Live Subtitles</button>
...
//             disabled={isLeaving}
//             className="flex items-center gap-1 px-6 py-2 bg-red-600 hover:bg-red-500 text-white transition-all disabled:opacity-60"
//           >
...
//     : active
//       ? activeClass || 'bg-transparent text-white hover:bg-neutral-600'
//       : 'bg-neutral-800/70 text-neutral-400 hover:bg-neutral-700 hover:text-white border border-transparent';

//   return (
//     <button type="button"
//       onClick={onClick}
//       className={`flex items-center gap-1 px-3 py-2 text-[10px] font-medium transition-all ${cls}`}
//     >
...
import {
  Mic, MicOff, Video, VideoOff, Monitor, MonitorOff, PhoneOff,
  Users, MessageSquare, Hand, Smile, Copy, UserPlus, X,
  AlertCircle, Layout, HandIcon, MessageSquareText,
} from 'lucide-react';
import MeetingParticipantTile from './Meetingparticipanttile';
import MeetingChat from './Meetingchat';
import ParticipantsPanel from './Participantspanel';
...
export default function MeetingRoom({ roomId, meeting, user, lobbyData }: MeetingRoomProps) {
  ...
  // ── Live subtitle & transcript state ────────────────────────────────────
  const [subtitlesEnabled, setSubtitlesEnabled] = useState(false);
  const [liveSubtitle, setLiveSubtitle] = useState<any>(null);
  ...
  const { startCapture, stopCapture } = useAudioCapture(localAudioStream, socket, roomId);
  ...
  }, [subtitlesEnabled, startCapture, stopCapture]);
  // ── End live subtitle state ─────────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const sock = io({
      path: '/api/socket/io',
      query: { userId: user?.id || 'anonymous', meetingRoom: roomId },
      transports: ['websocket', 'polling'],
      reconnection: true,
    });
    ...
    sock.on('transcription', (data: any) => {
      setLiveSubtitle(data);
      if (data.isFinal) {
        setTranscriptHistory((prev) => [
          ...prev,
          { ...data, id: Math.random().toString(36).slice(2), timestamp: Date.now() },
        ]);
      }
      setTimeout(() => setLiveSubtitle((curr: any) => (curr === data ? null : curr)), 8000);
    });

    sock.on('transcription-error', (err: { message: string }) => {
  ...
  return (
    ...
    >
      ...
      {liveSubtitle && (
        <div className="absolute bottom-24 left-1/2 -translate-x-1/2 z-50 w-full max-w-2xl px-4 pointer-events-none">
          <div className="bg-black/85 backdrop-blur-md text-white px-6 py-4 rounded-2xl shadow-2xl border border-white/10 animate-in fade-in slide-in-from-bottom-2">
      ...
      <div className="h-14 flex items-center justify-between px-4 bg-transparent backdrop-blur-md border-b border-neutral-800/50 flex-shrink-0 z-20 relative">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button"
            onClick={() => { setShowParticipants((v) => !v); setShowChat(false); setShowSubtitles(false); }}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm transition-colors ${showParticipants ? 'bg-blue-600 text-white' : 'bg-neutral-800 text-neutral-300 hover:bg-neutral-700'}`}
          >
            <Users className="size-4" />
            <span className="text-xs font-medium">{allParticipants.length}</span>
          ...
          <button type="button"
            onClick={() => { setShowChat((v) => !v); setShowParticipants(false); setShowSubtitles(false); setUnreadChats(0); }}
            className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm transition-colors ${showChat ? 'bg-blue-600 text-white' : 'bg-neutral-800 text-neutral-300 hover:bg-neutral-700'}`}
          >
            <MessageSquare className="size-4" />
            {unreadChats > 0 && (
              <span className="absolute -top-1 -right-1 size-4 bg-red-500 rounded-full text-[9px] text-white flex items-center justify-center font-bold">
                {unreadChats > 9 ? '9+' : unreadChats}
              </span>
            )}
          </button>

          {/* Transcript / Subtitles panel toggle */}
          <button type="button"
            onClick={() => { setShowSubtitles((v) => !v); setShowChat(false); setShowParticipants(false); }}
            className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm transition-colors ${showSubtitles ? 'bg-blue-600 text-white' : 'bg-neutral-800 text-neutral-300 hover:bg-neutral-700'}`}
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Side panels ── */}
        <AnimatePresence>
          {(showChat || showParticipants || showSubtitles) && (
            <motion.div
              initial={{ width: 0, opacity: 0 }}
              animate={{ width: 320, opacity: 1 }}
              exit={{ width: 0, opacity: 0 }}
      ...
      <div className="h-20 w-full flex items-center justify-center flex-shrink-0">
        <div className="flex justify-center items-center bg-black rounded-full !divide-y-0 !divide-x-2 !divide-[#222] overflow-hidden h-fit border border-[#333]">
          ...
          <button type="button"
            ...
            className={`flex bg-transparent items-center gap-1 px-1.5 py-2 text-[10px] font-medium transition-all ${subtitlesEnabled
                ? ' text-blue-400 border border-blue-500/30'
                : ' text-neutral-400 hover:bg-neutral-700 hover:text-white border border-transparent'
              }`}
          >
            <span className="size-4 leading-none bg-transparent">🌐</span>
