...
import { CloudSunIcon, Disc2, Plus } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Sun } from 'lucide-react';
import { FamilyButtonDemo } from '@/components/ui/multiButton';
import DashMusic from '../musics/dashMusic';
...
const Dashboard = () => {
    ...
    const fetchWeather = async () => {
      try {
        ...
        setWeather({
          ...
          wind: data.current_weather.windspeed,
          precipitation: data.current_weather.precipitation ?? 0,
          forecast: data.daily.temperature_2m_max.slice(0, 5).map((temp: number, index: number) => ({
            day: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][(new Date().getDay() + index) % 7],
            temp,
          })),
        });
      } catch (error) {
  ...
  return (
    <div className="flex dark:bg-black w-full h-screen max-h-screen overflow-y-hidden">
      {/* <Mainsidebar /> */}
      <div className="w-full h-full overflow-y-auto thin-scrollbar">
        {/* <Header /> */}
        <div className=" w-[100%] max-h-[100%] flex justify-start items-start">
          <Tabs defaultValue='Remindar' className="flex-[0.7] py-0 space-y-4 h-full min-h-full bg-gray-50 dark:bg-[#040404] flex flex-col justify-start items-start gap-0 !px-0 max-h-full  relative"
          >
            <div className="flex mx-auto items-center w-[80%] justify-between pt-4">

              <AnimateIcon animateOnHover className='!px-0 !py-0 '>
                <TabsList className="!px-0 !py-0 !h-fit !rounded-lg">
                  <TabsTrigger value="Remindar" className='rounded-r-none py-1.5'><AlarmClock /></TabsTrigger>
                  <TabsTrigger value="Notes" className='rounded-l-none py-1.5'> <ClipboardList /></TabsTrigger>
                </TabsList>
              </AnimateIcon>
              <Link
                href="/task/layout">
            ...
            <TabsContent value="Remindar" className="flex h-full flex-col w-[80%] !gap-y-4 mx-auto my-0 items-center   p-0">
              {isLoading ? (
              ...
              <FloatingPanelRoot className="w-full bg-transparent !px-0 !py-0">
                <FloatingPanelTrigger
            ...
            <TabsContent value="Notes" className="flex h-full flex-col w-[80%] !gap-y-4 mx-auto !my-0 items-center !px-0 !py-0">
              <NotesPage />

            </TabsContent>
            <div className="flex mx-auto items-center pt-0 w-[80%]">
              <Calendar
                mode="single"
                selected={date}
                onSelect={setDate}
          ...
          <div className="flex-[0.6] pt-5 px-4 justify-center items-center flex flex-col h-full">
            <div className="w-[94%] px-4 flex flex-col justify-center items-center ">
              <MusicProvider>
              ...
              <div className="relative group flex w-full max-h-full h-full min-h-full mt-4 shadow-md !gap-y-7 justify-center items-start px-6 bg-gray-50 dark:bg-neutral-950 pt-4 rounded-[14px] flex-col">
...
export function Remindars({ task, setTask }: TaskProps) {
  ...
  return (
    <div className="w-full overflow-y-scroll h-[26%] md:max-h-[26%] lg:max-h-[28vh] xl:max-h-[28vh] thin-scrollbar space-y-4">
        ...
        <div key={`task-${t.id}`} className="w-full">
          {i === 0 ? (
            <div
              ...
              className="bg-white dark:bg-[#111111] hover:shadow-md transition-all duration-300 justify-start items-start rounded-[12px] p-5 gap-4 w-full flex cursor-pointer group"
            >
            ...
            /* ✨ ALL OTHER CARDS */
            <div className="bg-white dark:bg-neutral-950 justify-between items-center rounded-[12px] p-3 gap-4 w-full flex group">
...
function NotesContent({
  ...
  return (
    <div className="mt-4">
      <div className="w-full overflow-y-scroll h-[26%] md:max-h-[26%] lg:max-h-[30vh] xl:max-h-[30vh] thin-scrollbar space-y-4">
        {notes.length === 0 ? (
          ...
          <div className="space-y-3">
            {notes.map((note: Note) => (
              <div
                ...
                className={`bg-white dark:bg-[#111111] hover:shadow-md border-none transition-all duration-300 justify-start items-start rounded-[12px] p-5 gap-4 w-full flex cursor-pointer group ${note.color ? `border-l-4 border-l-${note.color}` : ''}`}
              >
                <div className="relative h-10 w-10 flex-shrink-0">
                  <div className="flex justify-center items-center rounded-[8px] bg-yellow-400 dark:bg-[#2647eb] p-1 py-1.5">
                    {note.emoji || <Scroll className="dark:text-white h-5 w-5" />}
                  </div>
                </div>

                <div className="flex flex-col flex-1">
                  <p className="text-black dark:text-white font-bold text-[18px]">{note.title || 'Note'}</p>
                  <p className="text-gray-500 font-sans text-[14px] font-semibold">
                  ...
                  <p className="text-gray-400 font-medium text-[14px]">{note.preview}</p>
                </div>
      ...
      <Button
        ...
        className="w-full mt-4 !px-0 !py-0 min-w-full h-full bg-white border-none dark:bg-neutral-950 !rounded-[12px] justify-start items-center gap-4 flex"
      >
        <div className="bg-white dark:bg-neutral-950 justify-start items-center !rounded-[12px] p-3 gap-4 w-full flex">
          <div className="!h-10 !w-10 p-2 flex justify-center items-center rounded-[10px] bg-yellow-400 dark:bg-[#2647eb]">
