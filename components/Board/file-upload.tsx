...
export function FileUpload({ 
  ...
  return (
    <>
      {/* ONLY the upload button - no inline list */}
      <div className="flex items-center gap-3">
        <input
          ref={inputRef}
          type="file"
        ...
        <label
          htmlFor={`file-upload-${type}-${taskId || commentId}`}
          className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-1.5 cursor-pointer"
        >
          {isUploading ? (
            <Loader2 className="size-3.5 animate-spin" />
          ) : (
      ...
      {/* ONLY the toast - no inline upload list */}
